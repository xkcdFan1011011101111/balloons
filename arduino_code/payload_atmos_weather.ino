...
// set up BMP180 pressure sensor
#include <Wire.h>
#include <Adafruit_Sensor.h>
#include <Adafruit_BMP085_U.h>
Adafruit_BMP085_Unified bmp = Adafruit_BMP085_Unified(10085);

// set up HIH 4030 relative humidity sensor
#define relHumidPin 0 
...
void setup()  
{
  ...
  // connect to pc/openLog at 115200 so we can write fast enough without missing GPS measurements
  Serial.begin(115200);
  ...
  // print file header
  Serial.print("arduino_msec,gps_yr,gps_mo,gps_day,gps_hr,gps_min,gps_sec,gps_msec,");
  Serial.print("gps_fix,gps_qual,gps_num_sat,gps_latt,gps_long,gps_alt_meter,gps_speed_meter_per_sec,gps_ang_deg,");
  Serial.print("bmp_press_hPa,bmp_temp_degC,bmp_alt_meter,rel_humid_percent");
  Serial.println();
...
void loop()                     // run over and over again
{
  ...
  if (millis() - timer > 2000) { 
    ...
    if (GPS.fix){
      ...
      // get pressure, temp, and "altitude" from bmp180
      sensors_event_t event;
      bmp.getEvent(&event);
      // print pressure
      Serial.print(event.pressure);               Serial.print(','); 
      // get current temp from bmp180
      float temperature;
      ...
      // get relative humidity from hih4030
      float rel_humid_adc_cnt = (float)(analogRead(relHumidPin));// output is 0-1023 adc counts (corresponding to 0 - 5 volts)
      // analog-to-digital-converter (adc) reports counts; 0 counts <-> 0 v_adc, 1023 counts <-> 5 volts
      float rel_humid_percent = -25.806452 + 0.157664*rel_humid_adc_cnt - 0.028162 * temperature + 0.000334*rel_humid_adc_cnt*temperature;
      Serial.print(rel_humid_percent);
          
      // new line
      Serial.print('\n');
      // light activity LED to indicate we are getting GPS fixes
      ...
      // new line
      Serial.print('\n');
      // turn off activity LED to indicate we aren't getting GPS fixes
